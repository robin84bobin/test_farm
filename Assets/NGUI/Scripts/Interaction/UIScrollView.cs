...
public class UIScrollView : MonoBehaviour
{
	...
	protected bool mDragEnabled = true;

	public bool dragEnabled { set { mDragEnabled = value; } }
	...
	public virtual void UpdateScrollbars (bool recalculateBounds)
	{
		...
		if (horizontalScrollBar != null || verticalScrollBar != null)
		{
			...
			if (horizontalScrollBar != null && bmax.x > bmin.x)
			{
				...
				if (_customSize) {
					UpdateScrollBarCustom(horizontalScrollBar as UIScrollBar, false, true);
				} else {
			...
			if (verticalScrollBar != null && bmax.y > bmin.y)
			{
				...
				if (_customSize) {
					UpdateScrollBarCustom(verticalScrollBar as UIScrollBar, true, true);
				} else {
	...
	public virtual void SetDragAmount (float x, float y, bool updateScrollbars)
	{
		...
		if (_customSize) {
			bool vertical = movement == Movement.Vertical;
			float sizeContent = _contentWidgetSize - (vertical ? mPanel.finalClipRegion.w : mPanel.finalClipRegion.z);
			ox = oy = Mathf.Lerp(0f, -sizeContent, vertical ? y : x);
		}
	...
	//MS_ADD
	private float _contentWidgetSize;
	private bool _customSize = false;
	private Vector3 _scrollOffset;

	public void SetContentCustomSize(float size)
	{
		_contentWidgetSize = size;
		_customSize = true;
	}

	private void UpdateScrollBarCustom(UIScrollBar sb, bool vertical = true, bool inverted = false)
	{
		_scrollOffset = mTrans.localPosition;

		float offsetVal = vertical ? _scrollOffset.y : _scrollOffset.x;
		float sizeContent = _contentWidgetSize - (vertical ? mPanel.finalClipRegion.w : mPanel.finalClipRegion.z);
		float val = offsetVal / sizeContent;
		mIgnoreCallbacks = true;
		sb.value = inverted ? val : 1 - val;
		sb.barSize = 0.1f;
		mIgnoreCallbacks = false;
	}
}
