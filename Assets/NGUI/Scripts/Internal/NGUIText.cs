...
static public class NGUIText
{
	...
	/// <summary>
	/// When printing text, a lot of additional _messageViewData must be passed in. In order to save allocations,
	/// this _messageViewData is not passed at all, but is rather set in a single place before calling the functions that use it.
	/// </summary>
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol != null)
			{
				...
				bool canStartNewLine = (actualLineCount < maxLines && maxLines > 0) || maxLines == 0; // MS_ADD
				if (Mathf.RoundToInt(x + symbol.advance * fontScale) > rectWidth && canStartNewLine)
				{
					if (x == 0f) return;
			...
			else // No symbol present
			{
				...
				// Doesn't fit? Move down to the next line
				bool canStartNewLine = (actualLineCount < maxLines && maxLines > 0) || maxLines == 0; // MS_ADD
				if (Mathf.RoundToInt(x + w) > rectWidth && canStartNewLine)
				{
					if (x == 0f) return;
	...
	/// This function's _messageViewData is meant to be used for precise character selection, such as clicking on a link.
