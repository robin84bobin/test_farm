...
public class UIPopupListInspector : UIWidgetContainerEditor
{
	...
	void OnItemNormal (string spriteName)
	{
		RegisterUndo();
		mList.itemNormalSprite = spriteName;
		Repaint();
	}

	void OnHighlightSide (string spriteName)
	{
		RegisterUndo();
		mList.highlightSideSprite = spriteName;
		Repaint();
	}

	void OnItemNormalSide (string spriteName)
	{
		RegisterUndo();
		mList.itemNormalSideSprite = spriteName;
		Repaint();
	}

	void OnHighlightSideDown (string spriteName)
	{
		RegisterUndo();
		mList.highlightSideDownSprite = spriteName;
		Repaint();
	}
	
	void OnItemNormalSideDown (string spriteName)
	{
		RegisterUndo();
		mList.itemNormalSideDownSprite = spriteName;
		Repaint();
	}
	...
	void DrawAtlas()
	{
		if (NGUIEditorTools.DrawHeader("Atlas"))
		{
			...
			NGUIEditorTools.DrawPaddedSpriteField("DataItem normal", mList.atlas, mList.itemNormalSprite, OnItemNormal);
			NGUIEditorTools.DrawPaddedSpriteField("Highlight side", mList.atlas, mList.highlightSideSprite, OnHighlightSide);
			NGUIEditorTools.DrawPaddedSpriteField("DataItem normal side", mList.atlas, mList.itemNormalSideSprite, OnItemNormalSide);
			NGUIEditorTools.DrawPaddedSpriteField("Highlight side down", mList.atlas, mList.highlightSideDownSprite, OnHighlightSideDown);
			NGUIEditorTools.DrawPaddedSpriteField("DataItem normal side down", mList.atlas, mList.itemNormalSideDownSprite, OnItemNormalSideDown);
	...
	void DrawFont ()
	{
		if (NGUIEditorTools.DrawHeader("Font"))
		{
			...
			NGUIEditorTools.DrawProperty("HighLight Text Color", serializedObject, "highlightTextColor");
			...
			NGUIEditorTools.DrawPadding();
			NGUIEditorTools.SetLabelWidth(80f);
			NGUIEditorTools.DrawProperty("DataItem Horizontal Padding", serializedObject, "itemHorizontalPadding");
			GUILayout.EndHorizontal();
